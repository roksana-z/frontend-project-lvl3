...
const updateFeed = (state, proxyUrl) => new Promise((resolve) => {
  resolve(state.feeds.forEach((feedData) => {
    axios.get(`${proxyUrl}${feedData.url}`)
      .then((response) => {
  ...
  .then(() => setTimeout(() => updateFeed(state, proxyUrl), 5000));
...
export default () => {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (validationErrors.length > 0) {
      watchedState.feedsProcess.status = 'loading failed';
